...
{
    NSLog(@"MsgNotifyMsgNotifyMsgNotifyMsgNotify");
...
//    }
    ...
    //modified by wangmh
    if (m_WifiParamsGettingDelegate != nil) {
        [m_WifiParamsGettingDelegate WifiParams:[NSString stringWithUTF8String:szDID] enable:wifiParams.enable ssid:[NSString stringWithUTF8String:wifiParams.ssid] channel:wifiParams.channel mode:wifiParams.mode authtype:wifiParams.authtype encryp:wifiParams.encrypt keyformat:wifiParams.keyformat defkey:wifiParams.defkey strKey1:[NSString stringWithUTF8String:wifiParams.key1] strKey2:[NSString stringWithUTF8String:wifiParams.key2] strKey3:[NSString stringWithUTF8String:wifiParams.key3] strKey4:[NSString stringWithUTF8String:wifiParams.key4] key1_bits:wifiParams.key1_bits key2_bits:wifiParams.key2_bits key3_bits:wifiParams.key3_bits key4_bits:wifiParams.key4_bits wpa_psk:[NSString stringWithUTF8String:wifiParams.wpa_psk]];
    }
    [m_WifiParamsLock unlock];
}
...
void CPPPPChannel::ProcessCheckUser(char *pbuf, int len)
{
    ...
    if (result == 0)
    {
        ...
        MsgNotify(MSG_NOTIFY_TYPE_PPPP_STATUS, PPPP_STATUS_VALID_USER_PWD); //added by wangmh
    }
    else
    {
        //PPPPClose();
...
int CPPPPChannel::cgi_get_common(char * cgi)
{
    ...
    NSLog(@"cgi=[%s], m_bonline=[%d]", cgi, m_bOnline);
    if (m_bOnline == 0) {
        return 0;
    }
...
int CPPPPChannel::CheckUser(const char *user, const char *passwd){
//    NSLog(@"cgi=[%s], m_bonline=[%d]", cgi, m_bOnline);
    if (m_bOnline == 0) {
        return 0;
    }
    ...
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "GET /check_user.cgi?loginuse=%s&loginpas=%s&user=%s&pwd=%s&", user, passwd, user, passwd);
    cmdhead.len = strlen(buf);
...
int CPPPPChannel::RebootDevice(const char *user, const char *passwd){
    //    NSLog(@"cgi=[%s], m_bonline=[%d]", cgi, m_bOnline);
    if (m_bOnline == 0) {
        return 0;
    }
    ...
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "GET /reboot.cgi?loginuse=%s&loginpas=%s&user=%s&pwd=%s&", user, passwd, user, passwd);
    cmdhead.len = strlen(buf);
...
int CPPPPChannel::SetUserPwdForOther(const char *user, const char *passwd){
    if (m_bOnline == 0) {
        return 0;
    }
    ...
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "GET /set_users.cgi?user1=&user2=&user3=%s&pwd1=&pwd2=&pwd3=%s&loginuse=%s&loginpas=%s&user=%s&pwd=%s&", szUser, szPwd, user, passwd, user,passwd);
    cmdhead.len = strlen(buf);
...
void CPPPPChannel::SetUserAndPwd(const char *user, const char *passwd){
    memset(szUser, 0, sizeof(szUser));
    strcpy(szUser, user);
...
RE_CONNECT:
    ...
    MsgNotify(MSG_NOTIFY_TYPE_PPPP_STATUS, PPPP_STATUS_CONNECT_SUCCESS);
...
void CPPPPChannel::CheckOnline()
{
    
    if (m_bOnline == 1) {
        NSLog(@"CPPPPChannel::CheckOnline m_bOnline == 1 "); 
        MsgNotify(MSG_NOTIFY_TYPE_PPPP_STATUS, PPPP_STATUS_ON_LINE);
        return;
    }else{
        NSLog(@"CPPPPChannel::CheckOnline m_bOnline != 1 "); 
...
int CPPPPChannel::CheckOnlineReturn(){
            ...
            if (m_PlaybackViewImageNotifyDelegate != nil) {
                ...
                if (pH264Decoder->DecoderFrame((uint8_t*)pbuf, videoLen, nWidth, nHeight)) {
                    ...
                    if (pYUVBuffer != NULL) {
                        ...
                        delete[] pYUVBuffer;//modified by wangmh
                        pYUVBuffer = NULL;
                    }        
...
void CPPPPChannel::SetWifiParamsGettingDelegate(id delegate)
{
    [m_WifiParamsLock lock];
    m_WifiParamsGettingDelegate = delegate;
    [m_WifiParamsLock unlock];
}
...
void CPPPPChannel::SetOnlineFlag(int flag){
    if (flag == 2 && m_bOnline == 1) {
        m_bOnline = 2;
    }
}
